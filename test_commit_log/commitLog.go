...
func main() {
	...
	if true {
		for i := 0; i < 100; i++ {
			...
			log.Printf("开始读取文件%d----------------------------", i)
			// 读文件
			get_message, err := commitLog.ReadMessage(commitLogOffset)
			if err != nil {
				log.Fatal(err)
			}
			log.Printf("获得message: %+v", get_message.MsgId)
		}
	}

	if false {
		get_message, err := commitLog.ReadMessage(196608)
		if err != nil {
			log.Fatal(err)
		}
		log.Printf("获得message: %+v", get_message.MsgId)
